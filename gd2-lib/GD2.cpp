...
void GDClass::BitmapLayout(byte format, uint16_t linestride, uint16_t height) {
  ...
  b[3] = height;
  b[2] = (1 & (height >> 8)) | (linestride << 1);
  b[1] = (7 & (linestride >> 7)) | (format << 3);
  b[0] = 7;
  cI(c);
  if (ft8xx_model) {
    b[3] = (((linestride >> 10) & 3) << 2) | ((height >> 9) & 3);
    b[0] = 0x28;
    cI(c);
  }
}
void GDClass::BitmapSize(byte filter, byte wrapx, byte wrapy, uint16_t width, uint16_t height) {
  ...
  b[3] = height;
  b[2] = (1 & (height >> 8)) | (width << 1);
  b[1] = (3 & (width >> 7)) | (fxy << 2);
  b[0] = 8;
  cI(c);
  if (ft8xx_model) {
    b[3] = ((width >> 9) << 2) | (3 & (height >> 9));
    b[0] = 0x29;
    cI(c);
  }
}
...
void GDClass::ClearColorRGB(uint32_t rgb) {
  //printf("Clear Color command %lu", (2UL << 24) | (rgb & 0xffffffL));
...
//  printf("Clear command %lu", (38UL << 24) | 7);
...
void GDClass::get_inputs(void) {
  ...
  GDTR.daz_rd(wii_raw, sizeof(wii_raw));
    for (size_t i = 0; i < sizeof(wii_raw); i++) {
    printf("val: %li: %0X\r\n",i,wii_raw[i]);
  }
  if (wii_raw[0] == 0xda) {
